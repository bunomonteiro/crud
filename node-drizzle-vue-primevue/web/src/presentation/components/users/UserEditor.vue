...
async function onSave() {
  ...
  const response = await updateUserUC.handle({ username: props.user.username, patches: diff })

  if(response.error) {
    data.value.isSaving = false
    toast.add({ severity: 'error', summary: 'Usuário', detail: response.message, life: 10000 });
